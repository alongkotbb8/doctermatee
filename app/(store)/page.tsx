      ...
      {pinnedArticle && (
        <section className="reveal" style={{ padding: "16px 0 64px" }}>
          <div className="wrap">
            ...
            <div className="home-articles-mag" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, alignItems: "stretch" }}>
              ...
              {/* รายการขวา 3 ใบ — ภาพ cover เต็มความสูง ระยะห่างเท่ากันรอบด้าน */}
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                {otherArticles.map((a, i) => (
                  <Link key={a.id} href={`/articles/${a.slug}`} className={`anim-fade-up d${Math.min(i + 2, 5)}`} style={{ textDecoration: "none", flex: 1, display: "block" }}>
                    <article className="card pcard-hover" style={{ display: "flex", alignItems: "stretch", gap: 16, padding: 16, height: "100%" }}>
                      <div style={{ width: "40%", flexShrink: 0, borderRadius: 12, overflow: "hidden", background: "var(--neutral-50)", position: "relative" }}>
                        {a.cover_image ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={a.cover_image} alt={a.title} loading="lazy" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                        ) : (
                          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><IconPill size={30} color="var(--teal-300)" /></div>
                        )}
                      </div>
