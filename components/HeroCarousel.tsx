  ...
  return (
    ...
    >
      <div className="wrap grid-12" style={{ alignItems: "center", gap: 48, paddingTop: 60, paddingBottom: len > 1 ? 80 : 68 }}>
        {/* Left copy */}
        <div className="col-7" key={`copy-${i}`} style={{ animation: "fadeUp .55s var(--ease) both" }}>
        ...
        {/* Right visual */}
        <div className="col-5" key={`vis-${i}`} style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: 340, position: "relative", animation: "fadeIn .55s var(--ease) both" }}>
          {b.image ? (
      ...
      <style>{`@media (max-width: 600px){ .hero-arrow{ display:none !important } }`}</style>
    </section>
  );
}
